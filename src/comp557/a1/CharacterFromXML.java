...
public class CharacterFromXML {
	...
	public static GraphNode createJoint( Node dataNode ) {
		...
		if ( type.equals("free") ) {
			...
			if ( (t=getTuple3dAttr(dataNode,"degrees")) != null ) joint.setDegrees(t);
			Tuple2d minMaxRx;
			Tuple2d minMaxRy;
			Tuple2d minMaxRz;
			if ( (minMaxRx=getTuple2dAttr(dataNode,"minMaxRx")) != null ) joint.setMinMaxRx(minMaxRx);
			if ( (minMaxRy=getTuple2dAttr(dataNode,"minMaxRy")) != null ) joint.setMinMaxRy(minMaxRy);
			if ( (minMaxRz=getTuple2dAttr(dataNode,"minMaxRz")) != null ) joint.setMinMaxRz(minMaxRz);
			return joint;
			
		} else if ( type.equals("rotary") ) {
			// position and axis are required... passing null to set methods
			// likely to cause an exception (perhaps OK)
			...
			Double d;
			if ((d=getSingleDAttr(dataNode, "degree")) != null) joint.setDegree(d);
			Tuple2d minMax;
			if ( (minMax=getTuple2dAttr(dataNode,"minMax")) != null ) joint.setMinMaxDegree(minMax);
			return joint;
	 ...
	 * 
	 * TODO: Objective 6: Adapt commented code in createGeom to create your geometry nodes when loading from xml
	 */
	public static GraphNode createGeom( Node dataNode ) {
		...
		Tuple3d t;
		if ( type.equals("box" ) ) {
			BodyCube geom = new BodyCube( name );
	...
	/**
	 * Loads single double attribute of the given name from the given node.
	 * @param dataNode
	 * @param attrName
	 * @return
	 */
	public static Double getSingleDAttr(Node dataNode, String attrName) {
		Node attr = dataNode.getAttributes().getNamedItem(attrName);
		Double d = null;
		if (attr != null) {
			Scanner s = new Scanner(attr.getNodeValue());
			d = new Double(s.nextDouble());
			s.close();
		}
		return d;
	}
